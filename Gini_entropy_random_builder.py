...
def community_inequality_analysis(date):
    ...
    savelocation = f"/srv/abacus-1/bitcoin_darknet/grayscale_op_ali/heur_{options.heuristic}_data_v3/heur_{options.heuristic}_networks_full_shifted_random_community/{switcherback[options.frequency]}"
    ...
    networks_path = f"/srv/abacus-1/bitcoin_darknet/grayscale_op_ali/heur_{options.heuristic}_data_v3/heur_{options.heuristic}_networks_full_shifted/{switcherback[options.frequency]}"
    ...
    try:
        ...
        g = gt.GraphView(g, directed=True) # Get graph view for random version
    except OSError:
    ...
    g.vp["block"] = block_property_random
...
def process_timeunit(timeunit):
    ...
    for key, data_dict in data_dicts.items():
        ...
        file_path = os.path.join(f'jsonResults_v3/h{options.heuristic}/community_inequality_random', f'{key}_2009-01-03_{end_date}.json')
...
if __name__ == "__main__":   
    ...
    logging.basicConfig(level=logging.DEBUG, filename=f"logfiles/daily_weekly_final_heur_{options.heuristic}_v3/community_inequality_random_logfile", filemode="a+", format="%(asctime)-15s %(levelname)-8s %(message)s")
    chrono      = SimpleChrono()
